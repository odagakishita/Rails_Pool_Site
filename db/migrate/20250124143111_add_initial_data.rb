class AddInitialData < ActiveRecord::Migration[7.0]
  def up
    # 都市データの追加
    cities = [
      ...
      { name: "草ヶ崎市", slug: "chigasaki" },
      { name: "平塚市", slug: "hiratsuka" },
      { name: "鉄塚市", slug: "kamakura" },
      { name: "逢寺市", slug: "zushi" },
      { name: "三浦市", slug: "miura" },
    ...
    pools.each do |pool_data|
      ...
      if city
        ...
        Rails.logger.warn "City not found for slug: #{pool_data[:city_slug]}"
      end
    end

    # ユーザーデータの追加
    users = [
      { name: "小田城森太", email: "odagaki.job@gmail.com", password: "password", nickname: "shita" },
